...
{
    // ЗАЩИЩЕНО (10.09.2026, по мотивам реальной поломки в HREDU-183_tep_reports.js):
    // если LOG_NAME/CUR_OBJECT_ID ещё не настроены (например CUR_OBJECT_ID=0 -- заглушка
    // "TODO: заполнить после создания документа в админке"), вызов может упасть с ошибкой --
    // а LogAlert() вызывается ДО главного try/catch в Run(), поэтому необработанное
    // исключение тут рушило бы ВЕСЬ Run() целиком: RESULT никогда не устанавливался бы,
    // таблица оставалась бы пустой БЕЗ какого-либо сообщения об ошибке. Логирование --
    // вспомогательная вещь, её сбой не должен ронять сам отчёт.
    try
    {
 ...
 */
/*
 * ИСПРАВЛЕНО (15.09.2026, аварийное -- "Int(), Unknown source"): матрица может не иметь
 * собственного education_method_id (программы заданы только в её элементах
 * cc_learning_matrice_element -- законный, ожидаемый кейс, реальный тест пользователя
 * это подтвердил). Раньше "Int(This.education_method_id)" падал на Int(undefined).
 * Теперь -- OptInt(..., 0) + отбрасываем нули при сборке allProgramIds.
 */
function GetProgramIds(matrixRows, elementRows)
{
    ...
    {
        if (Int(matrixProgramIds[i]) > 0) { allProgramIds.push(matrixProgramIds[i]); }
    }
