...
//      фильтр аудитории для этого режима (сотрудник мог когда-то пройти программу, даже
//      если сейчас должность/мир-код уже не подходят под аудиторию её элементов). Базовый
...
//      равно применяются -- это их выбор, не аудитория.
...
//   а) ИЗМЕНЕНО (17.09.2026): аудитория теперь применяется НЕ к общему пулу сотрудников
//      ДО построения строк (раньше -- GetMatrixAudienceCollaboratorRows(), убрана), а К
//      КАЖДОЙ ГОТОВОЙ СТРОКЕ (сотрудник x программа) ПОСЛЕ построения -- потому что у
//      разных программ в одной и той же строке-сотруднике может быть РАЗНАЯ аудитория
//      (см. row.in_audience в BuildReportRows(), фильтр в Run() сразу после сборки RESULT).
//      Для result_type != "fact";
...
function Run()
{
    ...
    try
    {
        ...
        // ДОБАВЛЕНО (17.09.2026, HREDU-215 "Правки 1"): аудитория (по элементам, см.
        // row.in_audience в BuildReportRows()) применяется ЗДЕСЬ, К ГОТОВЫМ СТРОКАМ --
        // ДЛЯ ВСЕХ РЕЖИМОВ, КРОМЕ "fact" (та же логика, что была раньше на уровне пула
        // сотрудников -- "не зависимо от условий матрицы" для факта, см. "РЕШЕНИЯ" в шапке
        // файла). Раньше это было ДО построения строк (фильтр по единой аудитории матрицы),
        // теперь ПОСЛЕ -- потому что у разных программ в одной и той же строке-сотруднике
        // может быть РАЗНАЯ аудитория.
        if (sResultType != "fact")
        {
            filteredResultRows = [];
            ...
            LogAlert(1, "Run(). После фильтра аудитории по элементам матрицы осталось строк: " + ArrayCount(RESULT));
        }
