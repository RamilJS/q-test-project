...
function GetCurUserIdSafe()
{
    ...
    // ИЗМЕНЕНО (22.09.2026, ВТОРОЙ раунд -- ПЕРВЫЙ фикс, "не пропускать через
    // OptInt()/Int()", тоже не помог: userId=0 в логе даже без OptInt()). ПОДРОБНОЕ
    // логирование КАЖДОГО шага ниже -- по прямой просьбе пользователя ("сделай больше
    // логов"), чтобы СЛЕДУЮЩИЙ реальный тест сразу показал, на КАКОМ именно шаге всё
    // ломается, вместо очередной догадки. Также убрано сравнение "curUserId > 0"
    // (числовое сравнение) -- заменено на строковые проверки (String(...) ==/!=), т.к.
    // именно числовые операции на этом огромном (19-значном) id уже один раз подвели
    // (OptInt()). Если движок хранит curUserId НЕ как обычное число, а как что-то ещё
    // (например, строку, или собственный тип для больших id) -- оператор ">" мог вести
    // себя непредсказуемо; String()+сравнение строк -- самый проверенный в этом тикете
    // способ работы с такими значениями.
    try
    {
        bDefined = (typeof curUserId != "undefined" && curUserId != undefined);
    }
    catch (_exTypeof)
    {
        alert("GetCurUserIdSafe(). ДИАГНОСТИКА: даже typeof curUserId кинул ошибку: " + ExtractUserError(_exTypeof) + " -- возвращаем 0.");
        return 0;
    }

    alert("GetCurUserIdSafe(). ДИАГНОСТИКА: typeof curUserId=[" + typeof curUserId + "]; bDefined=" + bDefined);

    if (!bDefined)
    {
        alert("GetCurUserIdSafe(). curUserId НЕ определена (параметр не привязан в LPE?) -- возвращаем 0.");
        return 0;
    }
    ...
    {
        alert("GetCurUserIdSafe(). ДИАГНОСТИКА: String(curUserId) кинул ошибку: " + ExtractUserError(_exStr) + " -- возвращаем 0.");
        return 0;
    }

    alert("GetCurUserIdSafe(). ДИАГНОСТИКА: String(curUserId)=[" + sRaw + "]");

    if (sRaw == "" || sRaw == "0" || sRaw == "undefined" || sRaw == "null")
    {
        alert("GetCurUserIdSafe(). curUserId пустой/нулевой/некорректный (raw=[" + sRaw + "]) -- возвращаем 0.");
        return 0;
    }

    alert("GetCurUserIdSafe(). ВОЗВРАЩАЕМ curUserId как есть (raw=[" + sRaw + "]).");
    return curUserId;
}
