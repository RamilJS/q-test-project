...
//
// =====================================================================
// ВИДИМОСТЬ ПО РОЛИ (ДОБАВЛЕНО 22.09.2026, ШАГ 1 ИЗ 2):
//
// ТЗ ("Описание функционала отчета", п.1-2): отчёт доступен руководителям от
// руководителя группы и выше (видят по иерархии подчинённости) И сотрудникам УОРиАП
// (видят весь отчёт целиком). ПО ЯВНОМУ УКАЗАНИЮ ПОЛЬЗОВАТЕЛЯ (22.09.2026) СЕЙЧАС
// делаем ТОЛЬКО первую половину -- бинарный гейт:
//   - Сотрудник УОРиАП -> видит ВСЁ, что уже реализовано (без изменений остального кода).
//   - Любой другой сотрудник (включая руководителей) -> RESULT = [] (данные не
//     показываются вообще). Иерархия подчинённости для руководителей -- ОТДЕЛЬНЫЙ
//     следующий шаг, сейчас сознательно НЕ реализуется (по прямому указанию пользователя).
//
// ID ТЕКУЩЕГО ПОЛЬЗОВАТЕЛЯ: пользователь подтвердил (22.09.2026), что свяжет
// {{curUser.id}} в редакторе LPE этого виджета с параметром по имени "curUserId" --
// то есть в самом скрипте это ГОЛАЯ глобальная переменная curUserId (сам синтаксис
// {{curUser.id}} -- это разметка РЕДАКТОРА страниц, скрипту он недоступен напрямую).
// РИСК -- тот же паттерн, что уже один раз стоил "ничего не работает" с result_type в
// HREDU-183_breadcrumbs.js: если параметр вдруг НЕ привязан в LPE (например, при
// копировании виджета на другую страницу забыли перепривязать) -- обращение к
// необъявленной глобальной переменной curUserId кидает исключение. GetCurUserIdSafe()
// ниже защищена try/catch (по образцу GetRequestUrlSafe()) -- НЕ роняет весь Run(),
// просто вернёт 0, что через fail-safe в IsUorApMember() автоматически означает
// "доступа нет" -- то есть правильное поведение и для этого случая тоже (лучше по
// ошибке спрятать данные от того, кому должны быть видны, чем наоборот).
//
// ГРУППА УОРиАП: реальная структура подтверждена пользователем 22.09.2026 (прислал XML
// документа группы) -- код группы "uoriap_list" (поле code), список сотрудников --
// вложенная структура <collaborators><collaborator><collaborator_id>...
// </collaborator_id></collaborator>...</collaborators> (сейчас 14 записей). ВАЖНО,
// ОСТАЁТСЯ НЕПРОВЕРЕННЫМ ДО РЕАЛЬНОГО ТЕСТА: как называется тип документа в XQuery --
// "group" (как корневой элемент в самом XML) или "groups"? Во ВСЕХ прошлых каталогах
// этого тикета (collaborators, positions, cc_learning_matrices,
// cc_learning_matrice_elements) реальное имя типа в XQuery было МНОЖЕСТВЕННЫМ числом,
// даже когда сам объект называется в единственном числе -- поэтому ниже как ОСНОВНАЯ
// ГИПОТЕЗА взято "groups" (по аналогии, 100% совпадений в этом тикете на сегодня), но
// если реальный тест выдаст ошибку про неизвестный тип документа -- ПЕРВОЕ, что нужно
// попробовать поменять, это константа UORIAP_GROUP_XQUERY_TYPE ниже на "group".
//
// ДВА НЕЗАВИСИМЫХ СПОСОБА чтения вложенного списка сотрудников группы (IsUorApMember()
// пробует оба, логирует, какой сработал) -- путь в 2 уровня вложенности
// (collaborators/collaborator/collaborator_id) ЕЩЁ НЕ проверялся в этом тикете ни через
// XQuery (все прошлые XQuery использовали поля на ОДИН уровень, например $elem/name),
// ни через tools.open_doc() (все прошлые open_doc читали только простые одиночные поля,
// например matrixDoc.name) -- поэтому оба способа помечены как гипотезы, требующие
// проверки на реальном тесте.
// =====================================================================
//
// =====================================================================
// ВИДИМОСТЬ ПО ИЕРАРХИИ ДЛЯ РУКОВОДИТЕЛЕЙ (ДОБАВЛЕНО 23.09.2026, ШАГ 2 ИЗ 2):
//
// ТЗ, уточнено с пользователем 22-23.09.2026: руководитель (ЛЮБОЙ, у кого есть хотя бы
// один подчинённый -- "Руководитель группы" из ТЗ -- лишь ПРИМЕР конкретной должности, а
// не отдельный критерий) должен видеть отчёт, ОГРАНИЧЕННЫЙ ВСЕЙ ЦЕПОЧКОЙ ПОДЧИНЁННЫХ
// ВНИЗ -- свои прямые подчинённые + подчинённые своих подчинённых и так далее, сколько бы
// уровней ни было (подтверждено пользователем после разговора с тим-лидом). Работает КАК
// ВТОРОЙ, дополнительный гейт -- ПОСЛЕ того, как гейт УОРиАП (ШАГ 1) не пройден: сначала
// проверяем УОРиАП (видит всё), и ТОЛЬКО если это не так -- проверяем иерархию
// (руководитель видит подчинённых). Если ни то, ни другое -- RESULT = [] (прежнее
// fail-safe поведение, без изменений).
//
// ИСТОЧНИК ДАННЫХ ОБ ИЕРАРХИИ: подтверждено пользователем реальными карточками сотрудников
// (Рамиль -> Колесникова -> Соловьева -> Казначеев, 22.09.2026) -- у каждого сотрудника в
// документе collaborator есть вложенный список func_managers/func_manager, и РОВНО ОДНА
// запись в нём -- НЕПОСРЕДСТВЕННЫЙ руководитель (is_native=true). Остальные записи
// (is_native=false) -- функциональный/УЭД. Оценивающий/УЭД. Согласующий руководители --
// ПО ПРЯМОМУ УКАЗАНИЮ ПОЛЬЗОВАТЕЛЯ (22.09.2026) ИГНОРИРУЮТСЯ, иерархия строится ТОЛЬКО по
// is_native=true. У самого верхнего руководителя (в примере пользователя -- Казначеев) эта
// запись указывает САМА НА СЕБЯ (person_id руководителя == его же id) -- маркер вершины
// иерархии, см. защиту от самозацикливания в CollectDirectSubordinateIds()/
// GetManagerHierarchySubordinateIds() ниже.
//
// SQL/XQuery ДЛЯ ЧТЕНИЯ is_native: НАЙДЕНО ЧЕРЕЗ 6 РАУНДОВ РЕАЛЬНЫХ ТЕСТОВ (23.09.2026,
// см. временные диагностические файлы HREDU-182_test_manager_sql*.js) -- is_native хранится
// как ТИПИЗИРОВАННОЕ булево XML-поле (xs:boolean по схеме колонки c.data), а НЕ как
// обычный текст. Сравнение с ЛЮБЫМ строковым/числовым литералом (is_native=1,
// is_native="1", is_native=''1'', is_native=''true'' -- любая форма кавычек, любое
// значение, включая объективно правильное "true") валит ЗАПРОС ЦЕЛИКОМ (0 строк на ВСЮ
// таблицу, а не NULL в одном поле) -- платформа, похоже, глотает эту ошибку типов молча.
// РАБОЧИЙ синтаксис для типизированных булевых полей в SQL Server XQuery -- функция
// true()/false(), НЕ строковый литерал: is_native=true(). См. GetManagerIdRows() ниже --
// это и есть финальный, подтверждённый на 2312 строках запрос (раунд 6).
// ПОПУТНО ПОДТВЕРЖДЕНО (раунд 4): экспорт/просмотр документа показывает is_native как
// "0"/"1" -- РЕАЛЬНОЕ хранимое значение в сырой XML колонки c.data -- текст "false"/"true"
// (тот же класс сюрприза "экспорт документа ≠ то, что реально лежит в БД", что уже был с
// id группы УОРиАП, теперь подтверждён и для булевых полей). person_id ПРИХОДИТ из SQL
// ОБЫЧНЫМ ДЕСЯТИЧНЫМ ЧИСЛОМ (не hex-строкой "0x..." как в экспорте) -- конвертация не
// нужна.
//
// АЛГОРИТМ: BFS (обход в ширину) ВНИЗ по дереву подчинённости, БЕЗ рекурсивных вызовов
// функций (untested в этом движке -- по установившейся в этом тикете осторожной практике
// работаем только проверенными конструкциями) -- обычный массив-очередь (frontier),
// заполняемый через push(), с явной защитой от зацикливания (visited-список,
// пересортировываемый через SortIdArray() один раз НА УРОВЕНЬ BFS, а не на каждого
// сотрудника -- тот же принцип "сортировка один раз + бинарный поиск", что уже применяется
// везде в этом файле, см. комментарий над SortIdArray() и историю производительности
// 18.09.2026 -- иначе на большой организации риск повторить ту же O(n^2)-ловушку).
// Найденный список подчинённых используется, чтобы ОГРАНИЧИТЬ пул сотрудников (activeRows)
// в Run() -- бинарным поиском по отсортированному массиву id (IdArrayContainsSorted()),
// а не линейным перебором.
//
// ОТДЕЛЬНО, НЕ РЕАЛИЗУЕТСЯ СЕЙЧАС (идея тим-лида пользователя, озвучена 22.09.2026,
// пользователь явно отметил как неподтверждённую -- "но я пока не уверен, что это можно
// сделать"): сузить список матриц в фильтре страницы "Процент обученных" до тех, у чьих
// элементов мир-код совпадает с мир-кодами подчинённых конкретного руководителя. Не
// трогаем, пока не подтверждена работоспособность базового гейта по иерархии выше и пока
// пользователь явно не попросит вернуться к этой идее.
// =====================================================================

DEBUG = true;              // На проде поставить false после тестирования
LOG_NAME = "agent";        // TODO: заполнить после создания документа в админке
CUR_OBJECT_ID = 7685313676595870594;         // TODO: заполнить ID документа выборки после её создания в админке (LogAlert защищена try/catch -- забытый 0 не обрушит Run())
...
// УБРАНО (22.09.2026, по прямому указанию пользователя -- "выборка не работает"):
// LogAlert() дёргала внешнюю библиотеку tools.call_code_library_method("vtbl_log_lib",
// "LogAlert", ...) -- похоже, именно этот вызов и был источником проблемы (библиотека
// либо не подключена/не настроена для этого документа выборки, либо сама кидает ошибку
// в этом контексте). Try/catch внутри LogAlert() не давал этому уронить Run(), НО если
// сбой был не в самом логировании, а РАНЬШЕ (например, LogAlert() как таковая вообще
// была недоступна/не резолвилась в этом окружении) -- итог тот же: "ничего не
// работает" без диагностики. Все вызовы LogAlert(typeLog, message) по всему файлу
// заменены на alert(message) -- alert() объявлена в самом верху файла (обёртка над
// LogEvent(sLogName, sInputObj)), никакой внешней библиотеки не требует, ровно то же,
// что используется в PerfAlertSafe() ниже и во всех остальных файлах этого тикета для
