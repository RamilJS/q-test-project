...
/*
 * ИСПРАВЛЕНО (15.09.2026, аварийное -- "Int(), Unknown source"): реальный тест
 * пользователя показал матрицу, у которой ВООБЩЕ НЕТ собственного education_method_id
 * (поле в XML отсутствует -- обе программы заданы только через её 2 элемента, это
 * ЗАКОННЫЙ кейс, не ошибка данных). Старый код брал "Int(This.education_method_id)"
 * без защиты -- Int(undefined) падает с ошибкой. Теперь -- OptInt(..., 0) + отбрасываем
 * нули (0 -- не настоящий id программы, а признак "поле не заполнено").
 */
