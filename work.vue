...
// НАЙДЕНО (22.09.2026, ЧЕТВЁРТЫЙ раунд, реальный тест -- разгадка "typeof curUserId
// кинул ошибку" ИМЕННО внутри функции): пользователь добавил свой alert(curUserId) на
// ВЕРХНЕМ УРОВНЕ скрипта (не внутри функции) -- он сработал и напечатал правильное
// значение (7311507899656113337). А GetCurUserIdSafe() (ОБЫЧНАЯ function) в ТОМ ЖЕ
// САМОМ прогоне не смогла даже сделать typeof curUserId -- "Неизвестная ошибка". Вывод:
// curUserId, подставленная через LPE, судя по всему, существует ТОЛЬКО в области
// видимости верхнего уровня скрипта -- и НЕ видна изнутри обычных function-объявлений
// (в отличие от DEBUG/LOG_NAME/CUR_OBJECT_ID и т.п. -- они прекрасно читаются внутри
// функций весь этот тикет, но им ЯВНО присвоено значение прямо в этом файле оператором
// "=" без var, что делает их настоящими свойствами глобального объекта; curUserId же
// подставляется механизмом LPE, и, видимо, это НЕ то же самое, что обычное глобальное
// присваивание в этом движке). ФИКС: захватываем curUserId В ГЛОБАЛЬНУЮ ПЕРЕМЕННУЮ
// ПРЯМО ЗДЕСЬ, НА ВЕРХНЕМ УРОВНЕ (та же область видимости, где сработал alert
// пользователя) -- и уже эту переменную (CUR_USER_ID_RAW) читаем внутри
// GetCurUserIdSafe() и всех остальных функций. try/catch на случай, если на какой-то
// другой странице/копии виджета curUserId вообще не привязана в LPE -- тогда обращение
// к ней (даже на верхнем уровне) кинет исключение, и CUR_USER_ID_RAW останется undefined,
// что GetCurUserIdSafe() корректно обработает как "пользователь не определён".
CUR_USER_ID_RAW = undefined;
try
{
    CUR_USER_ID_RAW = curUserId;
}
catch (_exCaptureUserId)
{
    // curUserId не привязана в LPE на этой странице/копии виджета -- CUR_USER_ID_RAW остаётся undefined
}
...
function GetCurUserIdSafe()
{
    ...
    // ИЗМЕНЕНО (22.09.2026, ПЯТЫЙ раунд -- НОВАЯ находка): реальный тест показал, что
    // "typeof CUR_USER_ID_RAW" (а до этого и "typeof curUserId") САМИ КИДАЮТ исключение
    // ("Неизвестная ошибка"). Обычный typeof на неопределённой переменной в JS НИКОГДА
    // не кидает ошибку -- просто возвращает строку "undefined". Значит подозреваемый
    // теперь другой: сам ОПЕРАТОР typeof, похоже, НЕ ПОДДЕРЖИВАЕТСЯ этим движком (в этом
    ...
    // здесь не редкость). ФИКС: убираем typeof ПОЛНОСТЬЮ, используем ТОТ ЖЕ приём, что
    // уже проверенно работает в GetRequestUrlSafe() -- просто пытаемся ИСПОЛЬЗОВАТЬ
    // значение напрямую (String(...)) внутри try/catch, без предварительной проверки
    // типа. Пробуем ПО ОЧЕРЕДИ curUserId (напрямую, без всякой обвязки) и CUR_USER_ID_RAW
    // (копия, захваченная на верхнем уровне файла -- см. комментарий там) -- так этот же
    // тест сразу проверит ОБЕ оставшиеся гипотезы (viноват ли typeof как таковой, или всё
    // же область видимости).
    try
    {
        sRaw = String(curUserId);
        alert("GetCurUserIdSafe(). curUserId ДОСТУПНА напрямую (без typeof): [" + sRaw + "]");
    ...
    {
        alert("GetCurUserIdSafe(). curUserId напрямую НЕДОСТУПНА (ошибка: " + ExtractUserError(_exDirect) + ") -- пробуем CUR_USER_ID_RAW.");
    }

    try
    {
        sRaw = String(CUR_USER_ID_RAW);
        alert("GetCurUserIdSafe(). CUR_USER_ID_RAW (захвачена на верхнем уровне) ДОСТУПНА: [" + sRaw + "]");
        if (sRaw != "" && sRaw != "0" && sRaw != "undefined" && sRaw != "null")
        {
            return CUR_USER_ID_RAW;
        }
    }
    catch (_exCaptured)
    {
        alert("GetCurUserIdSafe(). CUR_USER_ID_RAW тоже НЕДОСТУПНА (ошибка: " + ExtractUserError(_exCaptured) + ").");
    }

    alert("GetCurUserIdSafe(). Ни curUserId, ни CUR_USER_ID_RAW не дали валидного значения -- возвращаем 0.");
    return 0;
}
...
{
    var rows, groupRows, iGroupId, groupDoc, collabList, i, sRawUserId;

    // ДОБАВЛЕНО (22.09.2026, по просьбе пользователя "сделай больше логов"): что именно
    // дошло до этой функции, и проверка "пусто/ноль" -- СТРОКОЙ, не числовым "<= 0"
    // (см. подробное объяснение в GetCurUserIdSafe() -- числовые операции на этом
    // 19-значном id уже один раз подвели).
    ...
    try
    {
        ...
        // непустой группе).
        // ИЗМЕНЕНО (22.09.2026, см. фикс в GetCurUserIdSafe() выше): сравниваем СТРОКАМИ
        // (String(...) == String(...)), а не через Int()/OptInt() -- на этих огромных
        // (19-значных) id только что нашёлся баг именно на числовой конвертации.
        // iCurUserId теперь приходит СЫРЫМ (без Int()), поэтому и вторую сторону
        // сравнения сознательно НЕ прогоняем через Int() -- строковое сравнение не
        // зависит от точности представления больших чисел вообще.
