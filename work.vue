...
//     Уточнено 10.09.2026 -- в тексте ТЗ
...
/*
 * Ищет макрорегион конкретного сотрудника. Идентична версии в HREDU-183_tep_reports.js.
 * ДОБАВЛЕНО (18.09.2026, по прямой просьбе пользователя): нужна, чтобы прокинуть РЕАЛЬНЫЙ
 * макрорегион города в drill-down ссылку на ТЭП (см. BuildTepLink() ниже) -- раньше в
 * ссылку клался только macroregion из РУЧНОГО ФИЛЬТРА страницы (sMacroregionFilter),
 * который чаще всего пуст (пользователь не обязан его выбирать) -- тогда на странице ТЭП
 * инфо-блок с текущими фильтрами (HREDU-183_182_info_makroregion_matrica.js) показывал
 * "Макрорегион: -", хотя у конкретного города, по которому кликнули, макрорегион на
 * самом деле есть и известен. Теперь берём его "по факту" -- у сотрудника, per которого
 * посчитана строка (город x программа), а не только из фильтра.
...
function Run()
{
    ...
    try
    {
        ...
        for (i = 0; i < ArrayCount(manualFilteredRows); i++)
        {
            ...
            sMacroregionForRow = FindMacroregion(macroRows, Int(manualFilteredRows[i].id));
            for (j = 0; j < ArrayCount(programIds); j++)
            {
