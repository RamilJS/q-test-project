    ...
    oForm.form_fields = [
        {
            ...
            // ИТОГ ДИАГНОСТИКИ (24.09.2026, реальный тест): query_qual в этом удалённом
            // действии РАБОТАЕТ КОРРЕКТНО в общем случае -- на диагностическом тесте с
            // catalog: "collaborator" и query_qual, ограниченным одним curUserId, пикер
            // показал РОВНО одного человека. Значит наш код (сбор subordinateIds,
            // relevantMirCodes, matrixIds, sMatrixQueryQual) полностью верен, а не
            // фильтрует именно каталог cc_learning_matrice -- то есть проблема на стороне
            // конфигурации ЭТОГО каталога в админке платформы (вероятно, у "Матрицы
            // обучения" настроен свой кастомный проводник выбора вместо стандартного,
            // который query_qual не учитывает). Дальнейшее исправление -- не в этом файле,
