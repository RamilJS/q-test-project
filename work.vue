 ...
 * Теперь -- OptInt(..., 0) + отбрасываем нули при сборке allProgramIds (0 -- это "поле
 * не заполнено", а не реальный id программы).
 */
 ...
 * @returns {Object[]}
 */
function BuildReportRows(collaborator, macroRows, cityRows, dateRows, programTitles, programIds)
{
...
function Run()
{
    ...
    try
    {
        ...
        for (i = 0; i < ArrayCount(collaboratorRows); i++)
        {
            collaboratorReportRows = BuildReportRows(collaboratorRows[i], macroRows, cityRows, dateRows, programTitles, programIds);
