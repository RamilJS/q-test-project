    ...
    switch(oRequest.workflow_state) {
        ...
        case "02": {
            ...
            if (OptInt(oRequestDoc.TopElem.custom_elems.ObtainChildByKey("f_mir_code_manager_id").value, 0) != 0) {
                if (IsEventWithoutDates(oRequestDoc))
                    tools.create_notification('accept_budget_mircode_manager_accept_2', OptInt(oRequestDoc.TopElem.custom_elems.ObtainChildByKey("f_mir_code_manager_id").value, 0), '', oRequestDoc.DocID, undefined, oRequestDoc.TopElem);
                else
