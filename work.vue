...
function GetCurUserIdSafe()
{
    ...
    // ИЗМЕНЕНО (22.09.2026, ЧЕТВЁРТЫЙ раунд -- НАЙДЕНА реальная причина, см. подробный
    // комментарий над CUR_USER_ID_RAW = curUserId; в шапке файла): curUserId,
    // подставленная через LPE, видна ТОЛЬКО на верхнем уровне скрипта, но НЕ внутри
    // обычных function-объявлений -- поэтому здесь читаем УЖЕ ЗАХВАЧЕННУЮ копию
    // CUR_USER_ID_RAW (обычная глобальная переменная, присвоенная на верхнем уровне --
    // такая же, как DEBUG/LOG_NAME/CUR_OBJECT_ID, которые всегда прекрасно читались
    // внутри функций весь этот тикет), а НЕ curUserId напрямую. Числовых сравнений
    // (">") по-прежнему нет нигде -- только String()-проверки, см. предыдущий раунд
    // фиксов (curUserId оказалась строкой, а не числом).
    try
    {
        bDefined = (typeof CUR_USER_ID_RAW != "undefined" && CUR_USER_ID_RAW != undefined);
    }
    catch (_exTypeof)
    {
        alert("GetCurUserIdSafe(). ДИАГНОСТИКА: даже typeof CUR_USER_ID_RAW кинул ошибку: " + ExtractUserError(_exTypeof) + " -- возвращаем 0.");
        return 0;
    }

    alert("GetCurUserIdSafe(). ДИАГНОСТИКА: typeof CUR_USER_ID_RAW=[" + typeof CUR_USER_ID_RAW + "]; bDefined=" + bDefined);

    if (!bDefined)
    {
        alert("GetCurUserIdSafe(). CUR_USER_ID_RAW НЕ определена (curUserId не привязана в LPE, либо захват на верхнем уровне не сработал) -- возвращаем 0.");
        return 0;
    }
    ...
    {
        alert("GetCurUserIdSafe(). ДИАГНОСТИКА: String(CUR_USER_ID_RAW) кинул ошибку: " + ExtractUserError(_exStr) + " -- возвращаем 0.");
        return 0;
    }

    alert("GetCurUserIdSafe(). ДИАГНОСТИКА: String(CUR_USER_ID_RAW)=[" + sRaw + "]");

    if (sRaw == "" || sRaw == "0" || sRaw == "undefined" || sRaw == "null")
    {
        alert("GetCurUserIdSafe(). CUR_USER_ID_RAW пустой/нулевой/некорректный (raw=[" + sRaw + "]) -- возвращаем 0.");
        return 0;
    }

    alert("GetCurUserIdSafe(). ВОЗВРАЩАЕМ CUR_USER_ID_RAW как есть (raw=[" + sRaw + "]).");
