...
function IsUorApMember(iCurUserId)
{
    ...
    try
    {
        ...
        LogAlert(1, "IsUorApMember(). Способ 1 (XQuery, вложенный путь) отработал без ошибок. userId=" + iCurUserId + " найдено записей: " + ArrayCount(rows));
        return (ArrayCount(rows) > 0);
    }
    catch (_exXQuery)
    {
        LogAlert(2, "IsUorApMember(). Способ 1 (XQuery, вложенный путь) кинул ошибку: " + ExtractUserError(_exXQuery) + " -- пробуем способ 2 (tools.open_doc())");
    }
    ...
    try
    {
        ...
        for (i = 0; i < ArrayCount(collabList); i++)
        {
            ...
            {
                LogAlert(1, "IsUorApMember(). Способ 2 (tools.open_doc()) сработал, совпадение найдено. userId=" + iCurUserId);
                return true;
            }
        }
        LogAlert(1, "IsUorApMember(). Способ 2 (tools.open_doc()) сработал, совпадений не найдено. userId=" + iCurUserId + "; записей в группе: " + ArrayCount(collabList));
        return false;
    }
    catch (_exDoc)
    {
        LogAlert(4, "IsUorApMember(). ОШИБКА: оба способа не сработали. Способ 2: " + ExtractUserError(_exDoc) + " -- fail-safe, считаем, что доступа НЕТ.");
        return false;
    }
}
...
function Run()
{
    ...
    try
    {
        ...
        {
            LogAlert(2, "Run(). Пользователь id=" + iCurUserId + " НЕ входит в группу УОРиАП -- данные не показываем (видимость по роли, шаг 1 из 2, руководители пока не реализованы).");
            RESULT = [];
